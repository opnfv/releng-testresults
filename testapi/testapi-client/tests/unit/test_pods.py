...
class PodTest(TestCommand):
    ...
    def setUp(self):
        ...
        self.config_mock.get_Instance().get_conf().get.return_value = 'http://localhost:8000/api/v1'
    ...
    def test_get(self, pod_get_http_mock):
        ...
        pod_get_http_mock.get_Instance().get.assert_called_once_with(self.base_url + '?name=dfs')
    ...
    def test_get_all(self, pod_get_http_mock):
        ...
        parsed_args = self.check_parser(pod_get, arglist, verifylist)
        pod_get.take_action(parsed_args)
        pod_get_http_mock.get_Instance().get.assert_called_once_with(self.base_url)
    ...
    def test_get_one(self, pod_get_http_mock):
        ...
        pod_get_http_mock.get_Instance().get.assert_called_once_with(self.base_url + '/def')

    @mock.patch('testapiclient.pods.HTTPClient')
    def test_create(self, pod_create_http_mock):
        pod_create = PodCreate(mock.Mock(), mock.Mock())
        arglist = ['{ "role": "community-ci", "name": "test_pod", "details": "", "mode": "metal"}']

        verifylist = [
            ('pod', {u'role': u'community-ci', u'name': u'test_pod', u'details': u'', u'mode': u'metal'})
        ]
        parsed_args = self.check_parser(pod_create, arglist, verifylist)
        pod_create.take_action(parsed_args)
        pod_create_http_mock.get_Instance().post.assert_called_once_with(self.base_url, None, verifylist[0][1])
    ...
    def test_create_unauthorized(self, pod_create_http_mock, pod_auth):
        ...
        arglist = [
            '-u', 'user', '-p', 'password',
            '{ "role": "community-ci", "name": "test_pod", "details": "", "mode": "metal"}']
    ...
    def test_create_authorized(self, pod_create_http_mock, pod_auth):
        ...
        parsed_args = self.check_parser(pod_create, arglist, verifylist)
        pod_create.take_action(parsed_args)
        pod_create_http_mock.get_Instance().post.assert_called_once_with(self.base_url, None, verifylist[2][1])
    ...
    def test_delete(self, pod_delete_http_mock):
        ...
        pod_delete_http_mock.get_Instance().delete.assert_called_once_with(self.base_url + '/def', None)
