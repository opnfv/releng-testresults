...
class PodGet(Command):
    ...
    def take_action(self, parsed_args):
        ...
        pods = httpClient.get(Config.config.get(Config.mode, "dev_api_url")+"/pods?name=" + parsed_args.name)
        print pods
...
class PodGetOne(Command):
    ...
    def take_action(self, parsed_args):
        httpClient = HTTPClient.getInstance()
        pods = httpClient.get(Config.config.get(Config.mode, "dev_api_url")+"/pods/" + parsed_args.name)
        print pods
    ...
    def take_action(self, parsed_args):
        ...
        if(parsed_args.u and parsed_args.p):
            ...
            if "login" in response.text:
                ...
                return
        response = httpClient.post(Config.config.get(Config.mode, "dev_api_url")+"/pods", User.session, parsed_args.pod)
    ...
    def take_action(self, parsed_args):
        ...
        if(parsed_args.u and parsed_args.p):
            ...
            if "login" in response.text:
                print "Authentication has failed. Please check your username and password."
                return
        pods = httpClient.delete(Config.config.get(Config.mode, "dev_api_url")+"/pods/" + parsed_args.name, User.session)
        print pods
