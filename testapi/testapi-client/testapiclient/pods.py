...
class PodGet(PodBase):
    ...
    def take_action(self, parsed_args):
        ...
        pods = httpClient.get(PodGet.pods_url + "?name=" + parsed_args.name)
        print pods
...
class PodGetOne(PodBase):
    ...
    def take_action(self, parsed_args):
        httpClient = HTTPClient.get_Instance()
        pods = httpClient.get(PodGetOne.pods_url + "/" + parsed_args.name)
        print pods
    ...
    def take_action(self, parsed_args):
        ...
        if(parsed_args.u and parsed_args.p):
            ...
            if "login" in response.text:
                ...
                return
        response = httpClient.post(PodCreate.pods_url, User.session, parsed_args.pod)
    ...
    def take_action(self, parsed_args):
        ...
        if(parsed_args.u and parsed_args.p):
            ...
            if "login" in response.text:
                print "Authentication has failed. Please check your username and password."
                return
        pods = httpClient.delete(PodDelete.pods_url + "/" + parsed_args.name, User.session)
        print pods
