...
class TestCaseCreate(TestCaseBase):
    @executor.create(http.client:c.BAD_REQUEST, message.no_body())
    def test_noBody(self):
        return None

    @executor.create(http.client:c.FORBIDDEN, message.no_permission())
    def test_unauthorized(self):
        self.project = 'newProject'
        return self.req_d

    @executor.create(http.client:c.FORBIDDEN, message.not_found_base)
    def test_noProject(self):
        self.project = 'noProject'
        return self.req_d

    @executor.create(http.client:c.BAD_REQUEST, message.missing('name'))
    ...
    @executor.create(http.client:c.OK, '_assert_success')
    ...
    @executor.create(http.client:c.FORBIDDEN, message.exist_base)
...
class TestCaseGet(TestCaseBase):
    ...
    @executor.get(http.client:c.NOT_FOUND, message.not_found_base)
    def test_notExist(self):
        return 'notExist'

    @executor.get(http.client:c.OK, 'assert_body')
    def test_getOne(self):
        return self.req_d.name

    @executor.get(http.client:c.OK, '_list')
    def test_list(self):
        return None
...
class TestCaseUpdate(TestCaseBase):
    ...
    @executor.update(http.client:c.BAD_REQUEST, message.no_body())
    def test_noBody(self):
        return None, 'noBody'

    @executor.update(http.client:c.NOT_FOUND, message.not_found_base)
    def test_notFound(self):
        update = tcm.TestcaseUpdateRequest(description='update description')
        return update, 'notFound'

    @executor.update(http.client:c.FORBIDDEN, message.exist_base)
    def test_newNameExist(self):
        self.create_e()
        return self.update_req, self.req_d.name

    @executor.update(http.client:c.FORBIDDEN, message.no_permission())
    def test_unauthorized(self):
        update_req_e = tcm.TestcaseUpdateRequest(project_name="newProject",
                                                 **self.req_e.format())
        return update_req_e, self.req_d.name

    @executor.update(http.client:c.FORBIDDEN, message.no_update())
    def test_noUpdate(self):
        update = tcm.TestcaseUpdateRequest(project_name=self.project,
                                           **self.req_d.format())
        return update, self.req_d.name

    @executor.update(http.client:c.OK, '_update_success')
    def test_success(self):
        return self.update_req, self.req_d.name
...
class TestCaseDelete(TestCaseBase):
    ...
    @executor.delete(http.client:c.NOT_FOUND, message.not_found_base)
    def test_notFound(self):
        return 'notFound', self.project

    @executor.delete(http.client:c.FORBIDDEN, message.no_permission())
    def test_unauthorized(self):
        return 'newCase', 'newProject'

    @executor.delete(http.client:c.UNAUTHORIZED, message.tied_with_resource())
    def test_deleteNotAllowed(self):
        self.create_help('/api/v1/results', self.results_d)
        return self.results_d.case_name, self.project

    @executor.delete(http.client:c.OK, '_delete_success')
    def test_success(self):
    ...
    def _delete_success(self, body):
        ...
        self.assertEqual(code, http.client:c.NOT_FOUND)
